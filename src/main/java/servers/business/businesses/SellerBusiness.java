...
import servers.business.beans.Seller;
import servers.business.vo.ScanInfo;
...
public class SellerBusiness extends Business<Seller>
{
    ...
    final static Logger logger = LogManager.getLogger(SellerBusiness.class);
    ...
    private void webRqstLoginQRCode(Session session)
    {
        ...
        JSONObject sendJson = new JSONObject();
        sendJson.put(Field.SCAN_ID, scanId);
        action.rspdClient(session, sendJson);
    }
    // Web用户请求扫码登录
    private void webQuickLogin(Session session)
    {
        ...
        JSONObject sendJson = new JSONObject();
        sendJson.put(Field.TOKEN,token);//登录令牌
        sendJson.put(Field.SELLER_INFO,JSONObject.toJSON(profile));
        action.rspdClient(session, sendJson);
    }
    // Web用户请求扫码登录
    ...
    {
        // 获取扫码ID
        String scanId = session.getRecvJson().getString(Field.SCAN_ID);
        ...
        if(scanInfo != null)
        {
            ...
            switch (scanInfo.getState())
            {
                case ScanCodeState.Scanned://验证通过,登录成功
                    ...
                    if (profile == null)
                    {//扫码成功但是用户数据为空
                        sendJson.put(Field.SCAN_STATE, ScanCodeResult.Fail);
                    }else{
                        sendJson.put(Field.SCAN_STATE, ScanCodeResult.Success);
                        String token = JwtHelper.createJWT(profileId);
                        sendJson.put(Field.TOKEN,token);//登录令牌
                        sendJson.put(Field.SELLER_INFO,JSONObject.toJSON(profile));
                    }
                ...
                case ScanCodeState.WaitScan://还在等待验证
                    if(System.currentTimeMillis() - scanInfo.getStartTime() > MAX_SCAN_WAIT_TIME)
                    {
                        ...
                        sendJson.put(Field.SCAN_STATE, ScanCodeState.Overdue);
                    }
                    else
                        sendJson.put(Field.SCAN_STATE, ScanCodeState.WaitScan);//继续等待
                    break;
    ...
    private void mpScanWebLogin(Session session)
    {
        // 获取扫码ID
        String scanId = session.getRecvJson().getString(Field.SCAN_ID);
        String scanResult = session.getRecvJson().getString(Field.SCAN_STATE);
        ScanInfo scanInfo = qrCodeHasScanned.get(scanId);
        JSONObject sendJson = new JSONObject();
        if(scanInfo != null)
        {
            ...
            {//扫码同意
                ...
                {// 扫码登录成功
                    ...
                    scanInfo.setScanProfileId(session.getRecvJson().getString(Field.profile_id));
                    sendJson.put(Field.SCAN_STATE,scanInfo.getState());
                }else{//二维码已经过期
                    sendJson.put(Field.SCAN_STATE,ScanCodeState.Overdue);
                }
            }else{//扫码拒绝
                sendJson.put(Field.SCAN_STATE,ScanCodeResult.Refuse);
            }
        }else{
            sendJson.put(Field.SCAN_STATE,ScanCodeState.Overdue);
        }
